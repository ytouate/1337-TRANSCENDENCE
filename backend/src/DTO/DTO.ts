...
export class ChatDto {
  ...
  @Matches(/^(?=.*\d)(?=.*\W+)(?=.*[A-Z])(?=.*[a-z]).{10,}$/, {
    message:
      'Password too weak. It must contain at least one uppercase letter, one lowercase letter, one digit, one special character, and be at least 10 characters long.',
  })
