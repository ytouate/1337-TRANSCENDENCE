    ...
    const interval = setInterval(() => {
      ...
      server.to(room).emit('game_update', {
        ball: {
          ...
          speedY: (ball.speedY * this.velocity * delta) / BOARD_HEIGHT,
        },
        gamePosition: {
          player1: players[0],
          player2: players[1],
  ...
  private async updateGameEnd(winnerId: number, loserId: number) {
    ...
    try {
      ...
      if (updatedGame.players[0].id === winnerId) {
        await this.userService.updateUserWin(winnerId, updatedGame.players[0]);
        await this.userService.updateUserLoss(loserId, updatedGame.players[1]);
      } else {
        await this.userService.updateUserWin(winnerId, updatedGame.players[1]);
        await this.userService.updateUserLoss(loserId, updatedGame.players[0]);
      }
    } catch (error) {
      throw error;
    }
